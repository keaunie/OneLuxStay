/* =========================
   LISTINGS PAGE – HOLLYWOOD
   ========================= */
function initListingsPageHollywood() {
  const root = document.getElementById("listing-root-hollywood");
  if (!root) return;
    ...
    for (const p of items) {
      ...
      card.innerHTML = `
        ...
        p.distance_km ? ` • ${p.distance_km} km from center` : ""
          ...
          <div class="cta">
            ${priceMarkup}
            <a class="btn btn--gold btn--price" href="/hollywoodProp?id=${encodeURIComponent(
              p.id
            )}" data-link>
              Show prices
            </a>
  ...
  const propsUrl = root.dataset.props || "/data/properties-hollywood.json";
  fetch(propsUrl, { cache: "no-store" })
    ...
    .catch((err) => {
      console.warn("Failed to load properties-hollywood.json:", err);
      state.all = [];
      wireControls();
      update();
    });
...
window.initListingsPageHollywood = initListingsPageHollywood;

/* =========================
   PROPERTY DETAIL PAGE – HOLLYWOOD
   ========================= */
function initPropertyDetailPageHollywood() {
  const container = document.getElementById("property-detail-hollywood");
  if (!container) return;

  const propsUrl = container.dataset.props || "/data/properties-hollywood.json";
  ...
  function notFound(msg = "Property not found.") {
    container.innerHTML = `<p style="padding:16px">${msg} <a href="/properties-hollywood" data-link>Back to listings</a></p>`;
  }
  ...
  function render(property, all) {
    ...
    container.innerHTML = `
      <a class="btn" href="/hollywood" style="color: black;" data-link>&larr; Back to listings</a>
      ...
      <div class="carousel-container">
        ...
        <h2 style="font-size:1.3rem;margin-bottom:0.5rem;">
          Your Private Hollywood Hideaway in the Sky
        </h2>
        <p>
          Live your Hollywood moment above the city, with front-row views of the
          legendary Hollywood Sign and a residence that feels more like a
          private members’ club than an apartment. This newly built
          3-bedroom sanctuary blends refined design, soft finishes and curated
          details to create a space that is both luxurious and effortlessly livable.
        </p>

        <p>
          By day, bathe the interiors in natural light as you work from the
          sophisticated co-working lounge and business center, complete with
          high-speed WiFi and a calm, professional atmosphere. By night, return
          to a quiet, elevated retreat just moments from the city’s most iconic
          landmarks and nightlife.
        </p>

        <h3 style="font-size:1.05rem;margin:1rem 0 0.4rem;">
          Designed for Effortless Living
        </h3>
        <p>
          Complimentary secure parking, a 24/7 fitness center, after-hours
          building security and thoughtfully designed shared spaces mean every
          detail is taken care of. Step outside to find Target just across the
          street, Starbucks a short stroll away, and an eclectic mix of dining
          and shopping options in the surrounding blocks.
        </p>
        ...
        <ul style="margin:0 0 1rem 1.2rem;padding:0;">
          <li>6 minutes to the Hollywood Walk of Fame</li>
          <li>10 minutes to Griffith Observatory</li>
          <li>10 minutes to Universal Studios</li>
          <li>20 minutes to The Grove</li>
        </ul>

        <p>
          Travelling with pets? Your four-legged companions are not just
          accepted, they’re welcomed. An on-site off-leash dog park and
          pet-washing station ensure they enjoy the Hollywood lifestyle
          alongside you.
        </p>

        <p style="margin-bottom:0;">
          Whether you’re here to create, explore or simply unwind above the
          city lights, this Hollywood residence offers a polished, modern
          haven in the heart of Los Angeles.
        </p>
      </div>
...
window.initPropertyDetailPageHollywood = initPropertyDetailPageHollywood;
