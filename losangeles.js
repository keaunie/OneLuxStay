/* =========================
   LISTINGS PAGE – LOS ANGELES
   ========================= */
function initListingsPageLosAngeles() {
  const root = document.getElementById("listing-root-losangeles");
  if (!root) return;
  ...
  const propsUrl = root.dataset.props || "/data/properties-losangeles.json";
  fetch(propsUrl, { cache: "no-store" })
    ...
    .catch((err) => {
      console.warn("Failed to load properties-losangeles.json:", err);
      state.all = [];
      wireControls();
      update();
    });
...
/* Expose LA init (don’t overwrite Antwerp’s initListingsPage) */
window.initListingsPageLosAngeles = initListingsPageLosAngeles;

/* =========================
   PROPERTY DETAIL PAGE – LOS ANGELES
   ========================= */
function initPropertyDetailPageLosAngeles() {
  const container = document.getElementById("property-detail-losangeles");
  if (!container) return;

  const propsUrl =
    container.dataset.props || "/data/properties-losangeles.json";
  ...
  function notFound(msg = "Property not found.") {
    container.innerHTML = `<p style="padding:16px">${msg} <a href="/properties-losangeles" data-link>Back to listings</a></p>`;
  }
  ...
  function render(property, all) {
    ...
    container.innerHTML = `
      <a class="btn" href="/losangeles" style="color: black;" data-link>&larr; Back to listings</a>
        ...
        .map(
          (sp) => `
            <div class="similar-card">
              ...
              <a class="book-btn" href="/losangelesProp?id=${encodeURIComponent(
              sp.id
            )}" data-link>View</a>
            </div>`
        )
  ...
  function initRoomModal(property) {
    ...
    function openRoom(room) {
      ...
      // Use the new room.images first, fall back to property.images
      const imgs =
...
/* Optionally expose detail init */
window.initPropertyDetailPageLosAngeles = initPropertyDetailPageLosAngeles;

/* ==========================================
   Listings utils (global, idempotent)
   ========================================== */
(function () {
  if (window.normalizeProperties) return; // already defined somewhere else
