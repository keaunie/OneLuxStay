/* =========================
   LISTINGS PAGE – MIAMI
   ========================= */
function initListingsPageMiami() {
  const root = document.getElementById("listing-root-miami");
  if (!root) return;
  ...
  const propsUrl = root.dataset.props || "/data/properties-miami.json";
  fetch(propsUrl, { cache: "no-store" })
    ...
    .catch((err) => {
      console.warn("Failed to load properties-miami.json:", err);
      state.all = [];
      wireControls();
      update();
    });
...
/* Expose MIAMI init */
window.initListingsPageMiami = initListingsPageMiami;

/* =========================
   PROPERTY DETAIL PAGE – MIAMI
   ========================= */
function initPropertyDetailPageMiami() {
  const container = document.getElementById("property-detail-miami");
  if (!container) return;

  const propsUrl = container.dataset.props || "/data/properties-miami.json";
  ...
  function notFound(msg = "Property not found.") {
    container.innerHTML = `<p style="padding:16px">${msg} <a href="/properties-miami" data-link>Back to listings</a></p>`;
  }
...
/* Expose MIAMI detail init */
window.initPropertyDetailPageMiami = initPropertyDetailPageMiami;
